...
public class OverlayBroadcaster {
	...
	public boolean reload() {
		if (overlays.isEmpty()) {
			...
			if (silenceTask != null) Sponge.getScheduler().getTaskById(silenceTask).ifPresent(Task::cancel);
	...
	public void hide() {
		silenceTask = Task.builder().delay(silenceInterval, TimeUnit.SECONDS).execute(this::forward).submit(plugin).getUniqueId();
	}
